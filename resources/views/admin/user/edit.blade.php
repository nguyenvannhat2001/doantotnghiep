...
@section('content')
         <form action="{{route('user.postedit',$user->id)}}" method="POST">
          ...
          <div>
          <label for="pwd">Loai tài khoản:</label>
          <select class="form-control" name="loaitaikhoan">
    <option value="admin" {{ $user->loaitaikhoan == 'admin' ? 'selected' : '' }}>Admin</option>
    <option value="user" {{ $user->loaitaikhoan == 'user' ? 'selected' : '' }}>User</option>
</select>
</div>
